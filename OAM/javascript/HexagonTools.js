...
HT.Hexagon = function(id, x, y) {
	...
	/*added following*/
	MidPointList.push(this.Id);
	MidPointList.push(this.x + (HT.Hexagon.Static.WIDTH / 2));
	MidPointList.push(this.y + (HT.Hexagon.Static.HEIGHT / 2));
	MidPointList.push(this.x1+x);
	MidPointList.push(this.y);
	MidPointList.push(this.x + HT.Hexagon.Static.WIDTH);
	MidPointList.push(this.y + HT.Hexagon.Static.HEIGHT);
...
HT.Hexagon.prototype.draw = function(ctx) {
	...
	{
		var p = this.Points[i];//http://localhost:4040/HexGrid/new-way.html
		ctx.lineTo(p.X, p.Y);
	}
	ctx.closePath();
	ctx.stroke();
	...
	if(this.Id)
	{
		...
		/*var img = new Image();
        img.src ="tower.jpg";
        ctx.drawImage(img,this.MidPoint.X-20,this.MidPoint.Y-20,30,30);
        ctx.fillText(this.Id, this.MidPoint.X, this.MidPoint.Y+20);*/
